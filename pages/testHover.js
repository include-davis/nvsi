...
const Popup = (setIsOpen) => {
    return (
        <>
            <div className={styles.colorBG} style={{backgroundColor: '#345707'}} onClick={() => setIsOpen(false)} />
...
const Test = () => {
    ...
    return (
        <>
            <button className={styles.primaryBtn} onMouseEnter={() => setIsOpen(true)} onmouseleave={() => setIsOpen(false)}>
                Open Modal
            </button>
            {isOpen && Popup(setIsOpen)}
        </>
    )
}
